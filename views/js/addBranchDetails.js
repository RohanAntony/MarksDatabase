app.controller('addBranchDetails',function($scope,$http){
  ...
  $scope.searchRecord = function(){
    if(!$scope.name){
      $scope.alertText = "Branch name is empty!!";
    }else{
  ...
  function verify(){
    if(!$scope.name){
      $scope.alertText = "Branch name is empty!!";
      pass = false;
    }else if($scope.yearList.length == 0){
      $scope.alertText = "Years list is empty!!";
      pass = false;
    }else if($scope.semList.length == 0){
      $scope.alertText = "Semester list is empty!!";
      pass = false;
    }
  }
